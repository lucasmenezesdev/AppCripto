...
class ContaRepository extends ChangeNotifier {
  ...
  comprar(Moeda moeda, double value) async {
    db = await DB.instance.database;
    await db.transaction((txn) async {
      //Verificar se a moeda já foi compradqa antes
      ...
      if (posicaoMoeda.isEmpty) {
        await txn.insert('carteira', {
          'sigla': moeda.sigla,
          'moeda': moeda.nome,
          'quantidade': (value / moeda.preco).toString()
        });
      }
      //Já tem a moeda em carteira
        ...
        await txn.update('carteira',
            {'quantidade': ((value / moeda.preco) + atual).toString()},
